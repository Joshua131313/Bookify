...
function Booksdisplay(props){
  ...
  // })
    const filteredfavs = favorites
     ?.filter(book => (
      pattern.test(book.title)||pattern.test(book.author) || keyword.toLowerCase()==='all'|| pattern.test(book.genre)||pattern.test(book.isbn)

    ))
    .map(book => <Book book={book} allbooks={allbooks} books={books} hidelike={hidelike}/>);
   const favsRow = filteredfavs?.length? filteredfavs.splice(0, limit): <Defaultmsg link='/create' text='Nothing here, create your book here!' icon='fal fa-layer-plus'/>
    const filtered = books
     ?.filter(book => (
      pattern.test(book.title)||pattern.test(book.author) || keyword.toLowerCase()==='all'|| pattern.test(book.genre)||pattern.test(book.isbn)
      ))
    .map(book => <Book allbooks={allbooks} book={book} books={books} editable={editable}/>);
      const booksRow = filtered?.length ? filtered.splice(0, limit) : <Defaultmsg link='/create' text='Nothing here, create your book here!' icon='fal fa-layer-plus'/>
  ...
  useEffect(()=>{
   ...
   useEffect(()=>{
    ...
    <Banner carousel={carousel} banner={banner} link='/create' btn='Create Book' text={'The magic of the internet is here! Easily manage your favorite books with bookify. You can even create your own collection of books!'} title={'Build your Library'}/>
...
{/*          
      ...
      <Loadingel El='div' size='100' classNames='trendingbooks'  info={favoritesbool?favsRow:booksRow} loader={loader}/>
      {determineBtns()}
